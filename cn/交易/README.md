...
其中，*VerificationCost*为虚拟机验证交易签名消耗的指令费用，*tx.Length*为交易数据的字节长度，*FeePerByte*为交易每字节的费用，目前为0.00001GAS。用户支付的网络费需要大于或等于此最小值，否则交易无法通过验证。
### attributes
...
验证脚本，常见为地址脚本，包括普通地址脚本，和多签地址脚本。也包括自定义的鉴权合约脚本。
...
## 交易签名
交易签名是对交易本身的数据（不包含签名数据，即witness部分）进行ECDSA方法签名。
